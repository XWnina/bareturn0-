...
public class CodeEvaluator : MonoBehaviour
{
    ...
    private readonly string[] answerPatterns = new string[]
    {
    ...
    private void TriggerNextDialogue()
    {
        ...
        {
            npcDialogueController.StartDoubleTeachingDialogue(currentQuestionIndex);
        }
    ...
    public string GetVariableValue()
    {
        ...
        try
        {
            if (currentQuestionIndex == 0) // `int` 类型
            {
                result =  CleanVariableExpression(userInput);
            }
            else if (currentQuestionIndex == 1) // `double` 类型
            {
                result = CleanVariableExpression(userInput);
            }
            else if (currentQuestionIndex == 2) // `float` 类型
            {
                result = CleanVariableExpression(userInput);
            }
            else if (currentQuestionIndex == 3) // 加法
            {
                result = EvaluateIntExpression(userInput);
            }
            else if (currentQuestionIndex == 4) // 减法
            {
                result = EvaluateIntExpression(userInput);
            }
            else if (currentQuestionIndex == 5) // 乘法
            {
                result = EvaluateIntExpression(userInput);
            }
            else if (currentQuestionIndex == 6) // 除法
            {
                result = EvaluateIntExpression(userInput);
            }
            else if (currentQuestionIndex == 7) // 模运算
            {
                result = EvaluateIntExpression(userInput);
            }
            else if (currentQuestionIndex == 8) // 自增
            {
                result = EvaluateIncrement(userInput);
            }
            else if (currentQuestionIndex == 9) // 自减
            {
                result = EvaluateDecrement(userInput);
            }
            else if (currentQuestionIndex == 10) // 混合运算
            {
    ...
    private string EvaluateIntExpression(string expression)
    {
        ...
        if (expression.Contains("="))
        {
            ...
            if (parts.Length == 2)
            {
                ...
                string[] nameParts = parts[0].Trim().Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
...
private int EvaluateExpression(string expression)
{
    ...
    try
    {
        ...
        if (expression.Contains("/"))
        {
            ...
            if (parts.Length == 2)
            {
                Debug.Log($"Parsed division: {parts[0].Trim()} / {parts[1].Trim()}");
                return int.Parse(parts[0].Trim()) / int.Parse(parts[1].Trim());  // 返回除法结果
            }
        }
        ...
        if (expression.Contains("%"))
        {
            ...
            if (parts.Length == 2)
            {
                Debug.Log($"Parsed modulo: {parts[0].Trim()} % {parts[1].Trim()}");
                return int.Parse(parts[0].Trim()) % int.Parse(parts[1].Trim());  // 返回取余结果
            }
        }
...
// 处理自减的函数
    ...
    private string EvaluateMixedExpression(string expression)
    {
        ...
        string[] lines = expression.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
        if (lines.Length == 3)
        {
            string var1Decl = lines[0].Trim(); // int apple = 3
            string var2Decl = lines[1].Trim(); // int pear = 4
            string sumDecl = lines[2].Trim();  // int sum = apple + pear
            ...
            // 处理 sum = a + b;
            string[] sumParts = sumDecl.Split(new char[] { '=' }, StringSplitOptions.RemoveEmptyEntries);
            string sumVarName = sumParts[0].Replace("int", "").Trim();

            string[] operands = sumParts[1].Split(new char[] { '+' }, StringSplitOptions.RemoveEmptyEntries);
...
private string CleanVariableExpression(string expression)
{
    ...
    {
        if (currentQuestionIndex >= answerPatterns.Length) return false;
        string pattern = answerPatterns[currentQuestionIndex];
    ...
    {
        // 显示最终对话
        npcText.text = "Incredible! You've now mastered all the basic ways to perform calculations in C.\nYou're ready to use the computer like a true coder!";
    ...
    {
        public int progress;
        public ProgressWrapper(int p) => progress = p;
    }
