...
router.put("/:saveName/setSelectedDeck", authMiddleware, async (req, res) => {
  try {
    ...
    if (!deck) {
      const errorResponse = {
        error: "Deck not found or does not belong to save file",
      };
      logRequestResponse(req, res, errorResponse);
      return res.status(404).json(errorResponse);
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const existingCard = saveFile.cardCollection.cards.find(
...
router.post(
  "/:saveName/removeCardFromCollection",
  authMiddleware,
  async (req, res) => {
    try {
      const { cardName, count } = req.body;
      ...
      const card = saveFile.cardCollection.cards.find(
        (c) => c.cardName === cardName
      );
      ...
      card.count -= count;
      if (card.count === 0) {
        saveFile.cardCollection.cards = saveFile.cardCollection.cards.filter(
          (c) => c.cardName !== cardName
        );
      }
